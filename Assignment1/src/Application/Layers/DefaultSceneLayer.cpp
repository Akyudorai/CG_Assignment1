...
#pragma region Shaders
		...
		ShaderProgram::Sptr noLightShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/no_light_frag.glsl" }
		});
		noLightShader->SetDebugName("No Light Shader");

		ShaderProgram::Sptr ambienceShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/ambience_frag.glsl" }
		});
		ambienceShader->SetDebugName("Ambience Shader");

		ShaderProgram::Sptr diffuseShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/diffuse_frag.glsl" }
		});
		diffuseShader->SetDebugName("Diffuse Shader");

		ShaderProgram::Sptr ambienceDiffuseShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/custom_frag.glsl" }
		});
		ambienceDiffuseShader->SetDebugName("Ambience Diffuse Shader");

		ShaderProgram::Sptr customShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/custom_frag.glsl" }
		});
		customShader->SetDebugName("Custom Shader");
...
#pragma endregion
		...
		MeshResource::Sptr goblinMesh = ResourceManager::CreateAsset<MeshResource>("goblin.obj");
		...
		Texture2D::Sptr goblinTex = ResourceManager::CreateAsset<Texture2D>("textures/goblin_uvs.png");
...
#pragma endregion 
		...
		Texture3D::Sptr coolLut = ResourceManager::CreateAsset<Texture3D>("luts/repImage.CUBE");
		Texture3D::Sptr warmLut = ResourceManager::CreateAsset<Texture3D>("luts/repImage.CUBE");
		Texture3D::Sptr customLut = ResourceManager::CreateAsset<Texture3D>("luts/repImage.CUBE");
...
#pragma region Materials
		...
		Material::Sptr customMat = ResourceManager::CreateAsset<Material>(customShader);
		{
			customMat->Name = "Custom";
			customMat->Set("u_Material.AlbedoMap", monkeyTex);
			customMat->Set("u_Material.NormalMap", normalMapDefault);
			customMat->Set("u_Material.Shininess", 0.5f);

			customMat->Set("u_Material.ToggleAmbience", true);
			customMat->Set("u_Material.AmbienceStrength", 0.5f);
			customMat->Set("u_Material.ToggleDiffuse", true);
			customMat->Set("u_Material.ToggleSpecular", true);
			customMat->Set("u_Material.SpecularStrength", 1.0f);
			customMat->Set("u_Material.ToggleInversion", false);

			customMat->Set("u_Light.Color", glm::vec3(1, 1, 1));
			customMat->Set("u_Light.Position", glm::vec3(0, -3, 5));
...
#pragma region Objects
		...
		{
			camera->SetPostion({ 0, -5, 2 });
			camera->LookAt(glm::vec3(0, 0, 2));

			//camera->Add<SimpleCameraControl>();
			camera->Add<FirstPersonCamera>();
		}
		...
		{
			light->SetPostion(glm::vec3(0, -3, 5));
		...
		GameObject::Sptr monkey = scene->CreateGameObject("Monkey");
		{
			...
			monkey->SetPostion(glm::vec3(0.0f, 0.0f, 2.0f));
			monkey->SetRotation({ 0, 0, 90 });
			...
			A1_Controller::Sptr controller = monkey->Add<A1_Controller>();
		}
